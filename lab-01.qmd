...
Unemployed_individuals <- ((Population * 0.2) * Unemployment_Rate)/100
Unemployed_individuals
```
...
class(GDP) # GDP
class(Inflation_Rate) # Inflation_Rate
class(Unemployment_Rate) # Unemployment_Rate
class(Population) # Population
class(GDP_per_capita) # GDP_per_capita
class(Unemployed_individuals) # Unemployed_individuals
...
**Sample covariance:** $$\frac{\sum(x_i - \overline{x})(y_i - \overline{y})}{n-1}$$

**Sample correlation:** $$\frac{\sum(x_i - \overline{x})(y_i - \overline{y})}{\sqrt{\sum(x_i - \overline{x})^2)(\sum(y_i - \overline{y})^2)}}$$
